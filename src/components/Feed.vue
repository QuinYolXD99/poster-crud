<template>
  <v-card max-width="80%" class="mx-auto">
    <v-list-item>
      <v-list-item-avatar>
        <v-avatar v-if="!details.user.account.avatar" color="indigo" size="48">
          <span class="white--text headline">{{details.user.account.username[0]}}</span>
        </v-avatar>
        <v-img v-else class="elevation-6" :src="details.user.account.avatar"></v-img>
      </v-list-item-avatar>
      <v-list-item-content>
    ...
    <v-img :src="details.images[0]" height="auto"></v-img>
    <v-card-text class="title black--text">
      ...
      <v-list-item-subtitle
        v-if="details.updatedAt"
      >Last Updated : {{`${details.updatedAt.month}/${details.updatedAt.date}/${details.updatedAt.year}`}}</v-list-item-subtitle>
    </v-card-text>
    <v-divider></v-divider>
    <v-card-actions v-if="details.user.account._id == this.user.account._id">
      ...
      <v-btn small icon @click="deletePost(details._id)">
